...
//
// The full type surface (MetronomeAudioParam/Oscillator/Gain/AudioContext and
// the public MetronomeEngine) lives in metronome-types.ts, which imports
// NOTHING — so type-only consumers (src/types.ts, and through it the browser
// cockpit's isolated typecheck) never reach this file's node-web-audio-api
// import. Everything is re-exported here, so existing
// `./playback/metronome.js` imports keep working unchanged.

import type { MetronomeAudioContext, MetronomeEngine } from "./metronome-types.js";
export type {
  MetronomeAudioParam,
  MetronomeOscillator,
  MetronomeGain,
...
// ─── Implementation ──────────────────────────────────────────────────────────
// (Public interface MetronomeEngine: see metronome-types.ts, re-exported above.)
