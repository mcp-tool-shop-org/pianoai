...
A mesma versão contém o motor de composição que alimenta o painel (`src/compose/`: um portão determinístico de condução de vozes com predefinições de estilo nomeadas, especificações de voz por pertencimento à construção, um refinador parte a parte), uma passagem completa de verificação de saúde (45 problemas corrigidos — moeda de segurança, cordas humanizadas, uma alteração visual que preserva a aparência), entradas da biblioteca Satie e Debussy reobtidas de bytes públicos com recibo da Mutopia e uma passagem de reforço do teste de estranhos: erros de validação descritivos, envelopes de erro `{code, message, hint}` estruturados, um arquivo tar selecionado, notificações de progresso em ferramentas longas e gramática de erro da CLI. A superfície ativa é **49 ferramentas e 4 modelos de prompt**, com **3.033 testes aprovados (1 ignorado)**. **Estado de publicação:** o mais recente do npm é **2.0.0** — tudo o que for mais novo está em `@mcptoolshop/ai-jam-sessions@2.2.0` até o lançamento da versão 2.2.0; execute a partir de um clone enquanto isso.
