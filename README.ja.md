...
<p align="center">
  AIにピアノとギターの弾き方と歌い方を教えるMCPサーバー。<br/>
  12ジャンル120曲。6つのサウンドエンジン。インタラクティブギタータブ譜。<br/>
  ボーカルシンセサイザー搭載のブラウザコックピット。すべてを記憶する練習日記。
</p>
...
AIが弾き方を学ぶピアノとギター。シンセサイザーでもMIDIライブラリでもなく、教育楽器です。
...
- **聞く** — 6つのオーディオエンジン（オシレーターピアノ、サンプルピアノ、ボーカルサンプル、物理声道モデル、加算合成ボーカル、物理モデルギター）がスピーカーで再生。部屋にいる人間がAIの耳になります。
- **見る** — 演奏内容をSVGとしてレンダリングするピアノロール。モデルが読み返して検証可能。インタラクティブギタータブ譜エディタ。ビジュアルキーボード、デュアルモードノートエディタ、チューニングラボを備えたブラウザコックピット。
...
6つのエンジンと、任意の2つを同時に実行するレイヤードコンビネーター：
...
| **ギター** | 加算合成 | 物理モデルの撥弦 — 4プリセット（スチールドレッドノート、ナイロンクラシカル、ジャズアークトップ、トゥエルブストリング）、8チューニング、17調整可能パラメーター。 |
...
4つのギターボイスプリセット。物理モデルの弦合成、17調整可能パラメーター（明るさ、ボディ共鳴、ピック位置、弦のダンピングなど）：

| ボイス | 特徴 |
|-------|------|
| Steel Dreadnought | 明るくバランスのとれたクラシックアコースティック |
| Nylon Classical | 温かく柔らかで丸みのある音 |
| Jazz Archtop | メロウで木の温もり、クリーン |
| Twelve-String | きらめくダブルトーン、コーラス効果 |
...
| ツール | 機能 |
|-------|------|
| `view_guitar_tab` | インタラクティブギタータブ譜をHTMLでレンダリング — クリック編集、再生カーソル、キーボードショートカット |
| `list_guitar_voices` | 利用可能なギターボイスプリセット |
| `list_guitar_tunings` | 利用可能なギターチューニングシステム（スタンダード、drop-D、open G、DADGADなど） |
| `tune_guitar` | ギターボイスのパラメーター調整。セッション間で保持 |
| `get_guitar_config` | ギターボイスの現在の設定 vs 工場出荷時デフォルト |
| `reset_guitar` | ギターボイスを工場出荷時にリセット |
...
v0.3.0。6つのサウンドエンジン、31のMCPツール、12ジャンル120曲（詳細なアノテーション付き模範曲）。インタラクティブギタータブ譜エディタ。20種のボーカルプリセット、10種のインストゥルメントボイス、7つの調律システム、LLM向けスコアAPIを備えたブラウザコックピット。2つのカラーモードのピアノロール。永続的な練習日記。MIDIはすべて揃っています——ライブラリはAIの学習とともに成長します。
