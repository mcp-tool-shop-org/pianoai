...
**実際に何かを学習しているのか？ — ファインチューニングの結果。** データセットの主張は、厳密な方法でテストされます。事前に登録されたファインチューニングの結果を、独自の固定ベースラインと比較し、トレーニングを開始する前に正直さのルールが適用されます。**v0**（78時間の音声データのみ）では、*正直なネガティブ*の結果が得られました。ツールを活用した質疑応答の精度は、0.661から0.601に低下しました（[レポート](docs/finetune-arc-eval-report.md)）。**v1**（実行検証済みのデータと、根拠に基づいた音声データを追加した494件のデータ）では、同じ指標が0.661から**0.863**に向上しました（+0.202、順列p = 0.0043、5つのシードすべてでベースラインを上回っており、未学習の曲では+0.433）。それでも、12/16回のペア比較で勝利したものの、事前に登録された目標である≥13/16に1つだけ届かなかったため、「全体的には改善されているが、まだ十分ではない」としてリリースされます（[レポート](docs/finetune-arc-v1-eval-report.md)）。わずかに目標に届かなかった結果からアダプターは公開されません。両方のデータセット、ロック、修正、およびシードごとの結果は、[`experiments/`](experiments/)に保存されています。重要なのは、この厳格なプロセスです。
...
**Node.js 22+**が必要です（v2.0.0で`node-web-audio-api` 2.0とともに最低要件が引き上げられました）。MIDIドライバー、仮想ポート、外部ソフトウェアは必要ありません。
...
v2.0.0 — データセットがその厳格さを証明したリリース（[CHANGELOG](CHANGELOG.md)を参照）。**重要な変更：Node.jsの最低バージョンは現在22です**（`node-web-audio-api` 2.0）。ツールの基本機能自体は変更されていません。6つのサウンドエンジン、46個のMCPツール、3つのプロンプトテンプレート、および**完全に注釈が付けられたライブラリ：12ジャンルにわたる120/120曲**（このリリースで、12個の主要なフィールドがコンテンツから検出されたキーに修正されました）。学習ループはエンドツーエンドで閉じられています。メトロノームによるカウントイン→ライブ録音→ノートごとの採点→採点済みのピアノロール→クリーンな演奏後にテンポを徐々に上げる練習ループ。ブラウザのインターフェースは、実際の作曲ツールです。正確なビートでの再生、ループ領域、録音開始、完全なアンドゥ/リドゥ、複数選択とクリップボード、タッチサポート—[ウェブ上で利用可能](https://mcp-tool-shop-org.github.io/ai-jam-sessions/cockpit/)。

また、**[jam-actions-v0](#training-dataset)**も公開します。これは、クラシックピアノにおける複数回のMCPツールの使用を記録した115件のトレーニングデータセットで、7軸のリリースゲート、コールドスタート時の再現性、および完全なZenodo + CITATION.cffメタデータ（CC-BY-SA-3.0-DE）を備えています。これは[Hugging Face](https://huggingface.co/datasets/mcp-tool-shop/jam-actions-v0)にもミラーリングされており、**両方向のファインチューニングの結果**が含まれています。正直なネガティブ（v0）と、事前に登録された目標に1つだけ届かなかったが、厳格なプロセスを経たポジティブ（v1）です—[ファインチューニングの結果](#training-dataset)を参照してください。このリリースでは、また、v1のパイプラインの実行ゲートが公開された範囲でBWV 846の実際の62小節を超過したことを検出し、バッハの曲をソースで修正しました（作業セットのリビジョンr001/r002と注記）。MCPサーバー+インターフェース+データセットパッケージャー+評価ハーネス+リリースゲートバリデーター全体で2506件のテストに合格しました。MIDIはすべて含まれており、すべての曲を学習に使用でき、その学習のコーパスも一緒に提供されます。
