...
- **Transporte preciso al compás:** las notas existen en el tiempo musical, por lo que el control de BPM realmente cambia la velocidad de reproducción; una regla de tiempo con clic para buscar y arrastrar para establecer **regiones de bucle**; desplazamiento automático que sigue la cabeza de reproducción.
- **Captura con grabación activada:** toque las teclas QWERTY, el teclado en pantalla o un dispositivo Web MIDI y se grabará en la partitura: 1 compás de introducción, sobregrabación estilo secuenciador a través de ciclos de bucle (o modo de reemplazo), preservación del tiempo de interpretación original bajo una vista cuantificada, cada pasada es una unidad que se puede deshacer.
...
- **Selección múltiple + portapapeles:** selección con herramienta Selecto/Dibujar, clics con modificadores estándar de la plataforma, copiar/cortar/pegar en la cabeza de reproducción, Duplicar.
...
- **Auditoría de la afinación:** tabla de frecuencias, probador de intervalos con análisis de frecuencia de batimiento y exportación/importación de la afinación.
...
|--------|------|---------------------|
| **Oscillator Piano** | Síntesis aditiva | Piano multiharmónico con ruido de martillo, inarmonicidad, polifonía de 48 voces, imagen estéreo. Cero dependencias. |
...
| `compose_panel` | Ejecutar el panel de composición de conducción de voces en cualquier canción: cuatro sistemas crean acompañamientos, un LLM ciego y de diferentes familias los clasifica, Bradley-Terry los agrega; con una puerta de discriminación que invalida las interpretaciones no interpretables (solo señal direccional, nunca una puntuación de calidad). |
...
Desde la versión 2.1.0, `main` también ha ampliado el motor de composición (`src/compose/`): una puerta determinista de conducción de voces con presets de estilo nombrados, especificaciones de voicing por construcción, un refinador nota a nota y la herramienta `compose_panel` que ejecuta un panel de clasificación ciego entre diferentes familias. La interfaz en vivo tiene **49 herramientas y 4 plantillas de indicaciones**, con **2930 pruebas superadas (1 omitida)**. **Estado de publicación:** la última versión de npm es **2.0.0**: todo lo de la versión 2.1.0 se encuentra solo en `main`; ejecútelo desde un clon hasta la próxima versión.
