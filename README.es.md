...
- **Audición:** seis motores de audio (piano oscilador, piano de muestras, muestras vocales, tracto vocal físico, sintetizador vocal aditivo, guitarra modelada físicamente) que se reproducen a través de sus altavoces, para que los humanos en la sala se conviertan en los oídos de la IA.
- **Visión:** un teclado de piano que renderiza lo que se tocó como SVG, para que el modelo pueda leerlo y verificarlo. Un editor interactivo de tablaturas de guitarra. Un panel de control del navegador con un teclado visual, un editor de notas de doble modo y un laboratorio de afinación.
- **Memoria:** un diario de práctica que persiste entre sesiones, para que el aprendizaje se acumule con el tiempo.
- **Canto:** síntesis del tracto vocal con 20 preajustes de voz, desde soprano operística hasta coro electrónico. Modo de canto a dúo con solfeo, contorno y narración silábica.
...
- **Modo Panel: la sala de escucha**: pruebas A/B ciegas por pares de las voces del motor de composición sobre melodías reales de la biblioteca: fragmentos con niveles de volumen iguales, renderizados offline a través de la ruta de voz real, pruebas aleatorias con controles ocultos para determinar el umbral de discriminación, clasificaciones Bradley-Terry con intervalos de confianza bootstrap y resultados honestos (PROVISIONALES hasta que cada par alcance su presupuesto de votos; NO INTERPRETABLES cuando falla el umbral de discriminación). Un segundo submodo ejecuta la misma clasificación con jueces LLM locales, además del historial de ambos tipos de ejecuciones y una vista de comparación (Kendall τ + coincidencia de rango del motor) que pregunta si las pistas proxy económicas reflejan la verdad humana.
...
- **Selección múltiple + portapapeles**: selección mediante un cuadro delimitador bajo un interruptor de herramienta Seleccionar/Dibujar, clics con modificadores estándar de la plataforma, copiar/cortar/pegar en el indicador de reproducción, duplicar.
- **Interfaz táctil y accesibilidad**: eventos del puntero con captura en cada superficie, tocar para reubicar como una alternativa que no requiere arrastrar, edición de notas por teclado, superposiciones de partituras seguras para personas con daltonismo.
- **Piano roll de doble modo**: cambia entre el modo Instrumento (colores de clase cromática) y el modo Vocal (notas coloreadas según la forma de la vocal: /a/ /e/ /i/ /o/ /u/).
- **Teclado visual**: dos octavas desde C4, asignado a tu teclado QWERTY. Haz clic o escribe.
- **20 preajustes de voz**: 15 voces mapeadas por Kokoro (Aoede, Heart, Jessica, Sky, Eric, Fenrir, Liam, Onyx, Alice, Emma, Isabella, George, Lewis, además de coro y voz sintetizada), 4 voces mapeadas por tracto y una sección de coro sintético.
- **10 preajustes de instrumento**: las 6 voces de piano del lado del servidor más pad de sintetizador, órgano, campana y cuerdas.
- **Inspector de notas**: haz clic en cualquier nota para editar la velocidad, la vocal y el timbre.
...
| **Sample Piano** | Reproducción de muestras. | Piano Salamander Grand: el sonido real. **El motor predeterminado cuando se instala un paquete** (`samples/AccurateSalamander` o `AI_JAM_SAMPLES_DIR`); el archivo tar de npm permanece sin muestras, por lo que proporcionas la descarga de [Salamander](https://freepats.zenvoid.org/Piano/acoustic-grand-piano.html). El panel del navegador incluye su propio paquete optimizado de 8 MB (90 archivos OGG, CC-BY 3.0 Alexander Holm), sin necesidad de configuración en la web. |
...
| `compose_panel` | Ejecuta el panel de composición de voces en cualquier canción: cuatro sistemas crean acompañamientos, jueces LLM ciegos de diferentes familias los clasifican, Bradley-Terry agrega los resultados, con una puerta de umbral de discriminación que invalida las ejecuciones no interpretables (solo señal direccional, nunca una puntuación de calidad). Se ejecuta durante minutos y muestra notificaciones de progreso mientras trabaja. |
...
**v2.2.0: la versión en la que el instrumento obtuvo oídos reales y una sala de escucha** (consulta [CHANGELOG](CHANGELOG.md)). El piano predeterminado del panel es ahora un **piano de concierto con sonido muestreado**: un paquete Salamander optimizado que se carga la primera vez que interactúas con él y vuelve al sintetizador oscilador afinado hasta que esté listo, y el servidor selecciona automáticamente el motor de muestra cuando se instala un paquete completo. Encima se encuentra el **Panel de composición**: una sala de escucha A/B ciega con niveles de volumen iguales donde un humano clasifica las voces del motor de composición en comparación con anclas teóricamente válidas e inválidas (Bradley-Terry con intervalos de confianza bootstrap, un umbral de discriminación estilo MUSHRA, PROVISIONAL y NO INTERPRETABLE como resultados de primer nivel), junto a un panel de modelos locales que ejecuta la misma clasificación con jueces LLM de diferentes familias y una vista de comparación (Kendall τ) que pregunta si las pistas proxy económicas reflejan la verdad humana.

La misma versión incluye el motor de composición que alimenta el panel (`src/compose/`: una puerta determinista de conducción de voces con preajustes de estilo nombrados, especificaciones de voz basadas en la pertenencia por construcción, un refinador parte a parte), una revisión completa de la salud (45 problemas solucionados: moneda de seguridad, cuerdas humanizadas, una modificación visual que conserva el aspecto), entradas de biblioteca de Satie y Debussy obtenidas de bytes públicos con licencia de Mutopia y una fase de endurecimiento de pruebas extrañas: errores de validación descriptivos, envolventes de error estructuradas `{code, message, hint}`, un archivo tar curado, notificaciones de progreso en herramientas largas y gramática de error de CLI. La superficie activa es **49 herramientas y 4 plantillas de indicaciones**, con **3033 pruebas aprobadas (1 omitida)**. **Estado de publicación**: la última versión de npm es **2.0.0**: todo lo más reciente se encuentra en `main` hasta el lanzamiento de la versión 2.2.0; ejecútalo desde un clon mientras tanto.
