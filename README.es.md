...
- **Captura con grabación activada:** toque las teclas QWERTY, el teclado en pantalla o un dispositivo Web MIDI y se guardará en la partitura: 1 compás de introducción, sobregrabación estilo secuenciador a través de ciclos de bucle (o modo de reemplazo), tiempo de interpretación sin procesar conservado bajo una vista cuantificada, cada pasada es una unidad que se puede deshacer.
...
- **Teclado de piano de doble modo:** cambie entre el modo Instrumento (colores cromáticos) y el modo Vocal (notas coloreadas por forma de vocal: /a/ /e/ /i/ /o/ /u/).
...
- **7 sistemas de afinación:** temperamento igual, entonación justa (mayor/menor), pitagórico, cuarto coma de intervalo, Werckmeister III u offsets de centavos personalizados. Referencia A4 ajustable (392–494 Hz).
...
|-------|----------|-----|-----------------|
| Blues | The Thrill Is Gone (B.B. King) | Si menor | Forma de blues menor, llamada y respuesta, tocar después del compás |
| Clásico | Für Elise (Beethoven) | La menor | Forma de rondó, diferenciación del tacto, disciplina en el uso del pedal |
...
| Twelve-String | Brillante, duplicada, tipo chorus |

## El diario de práctica

Después de cada sesión, el servidor captura lo que sucedió: qué canción, a qué velocidad, cuántas medidas, cuánto duró. La IA agrega sus propias reflexiones: lo que notó, los patrones que reconoció, qué probar a continuación.
...
Un archivo Markdown por día, almacenado en `~/.ai-jam-sessions/journal/`. Legible para humanos, solo se agregan entradas. En la siguiente sesión, la IA lee su diario y continúa donde lo dejó.

## Conjunto de datos de entrenamiento

**jam-actions-v0**: un conjunto de datos público de trazas de uso de herramientas MCP multivuelta basadas en MIDI reales de piano clásico. Creado a partir de la misma biblioteca con la que este servidor enseña, el conjunto de datos enseña a los LLM a realizar **uso de herramientas fundamentado sobre música simbólica**, no solo generación de texto.
...
**Historia de calidad: la puerta de liberación de 7 ejes.** El conjunto de datos incluye una puerta de liberación que distingue entre el razonamiento basado en pruebas y el rendimiento saturado. Los ejes 1 a 6 son restrictivos (límite absoluto, margen compuesto, tasa de uso de herramientas, corrección después del uso de la herramienta, recuento de interpretaciones erróneas, límite inferior); el eje 7 es enriquecido frente a no enriquecido. Los ejes 2 y 6 admiten un grupo `ceiling_saturated_pass`, por lo que los registros que obtienen una puntuación de 1.000 en las condiciones de solo texto / inspección con herramientas / MIDI aleatorio no diluyen los estratos más difíciles. La línea base Slice 22 **APRUEBA** la puerta revisada. La línea base Slice 19 aún **NO LA APRUEBA**, y se conserva como un diagnóstico de regresión para que la puerta sea efectiva.
...
**Dónde encontrarlo.** El registro publicado en Zenodo está disponible en https://zenodo.org/records/20279919 (DOI: [`10.5281/zenodo.20279919`](https://doi.org/10.5281/zenodo.20279919)), y el conjunto de datos está replicado en Hugging Face en [`mcp-tool-shop/jam-actions-v0`](https://huggingface.co/datasets/mcp-tool-shop/jam-actions-v0) para los usuarios de `load_dataset()`. La tarjeta completa del conjunto de datos está disponible en [`datasets/jam-actions-v0-public/README.md`](datasets/jam-actions-v0-public/README.md). Los metadatos de depósito de Zenodo están disponibles en [`zenodo-metadata.json`](datasets/jam-actions-v0-public/zenodo-metadata.json), los metadatos de citación en [`CITATION.cff`](datasets/jam-actions-v0-public/CITATION.cff), el comprobante de publicación en [`publication-receipt.json`](datasets/jam-actions-v0-public/publication-receipt.json) y las notas de la versión en [`RELEASE_NOTES.md`](datasets/jam-actions-v0-public/RELEASE_NOTES.md). El arco de construcción de 25 secciones, desde el borrador inicial del corpus hasta la corrección de un error, la remediación de Schumann, la revisión de la puerta RC, la auditoría de "operador solo" y la ejecución de la publicación, se encuentra en [`docs/`](docs/).
...
**¿Realmente entrena algo? — los comprobantes de ajuste fino.** Las afirmaciones del conjunto de datos se prueban a fondo: los ajustes finos preregistrados se evalúan en comparación con su propia línea base sellada, y las reglas de honestidad se congelan antes de cualquier entrenamiento. **v0** (las 78 secciones de jam solas) arrojó un *resultado negativo honesto*: la evaluación de calidad basada en herramientas disminuyó de 0.661 a 0.601 ([informe](docs/finetune-arc-eval-report.md)). **v1** (un conjunto de datos de 494 ejemplos que agrega secciones con trazabilidad verificada por ejecución y con forma basada en el razonamiento) movió la misma métrica de 0.661 a **0.863** (+0.202, permutación p = 0.0043, las cinco semillas por encima de la línea base, la canción no vista +0.433), y aún se ofrece como *"direccionalmente mejor, con poca potencia"* porque 12 de 16 victorias en pares no alcanzaron la barra preregistrada de ≥13/16 por una sola ([informe](docs/finetune-arc-v1-eval-report.md)). No se publica ningún adaptador a partir de un resultado casi exitoso. Ambos arcos, bloqueos, modificaciones y comprobantes por semilla se encuentran en [`experiments/`](experiments/) — la disciplina es el objetivo.

> Los arreglos MIDI son obra de Bernd Krueger (piano-midi.de), con licencia CC-BY-SA-3.0-DE. Las anotaciones, los registros y los artefactos de evaluación son del equipo de AI Jam Sessions, publicados bajo la misma licencia para que se conserve la cadena de "compartir y modificar". **Límite de licencia:** la licencia MIT del repositorio cubre el código; todo lo que se encuentra en `datasets/` tiene licencia CC-BY-SA-3.0-DE. El corpus de trabajo en `datasets/jam-actions-v0/` contiene además dos obras (Satie Gymnopédie No. 1, Debussy Arabesque No. 1) que están *excluidas* del subconjunto publicado porque no se pudo verificar la procedencia de su arreglo; consulte [`datasets/jam-actions-v0/PROVENANCE-NOTE.md`](datasets/jam-actions-v0/PROVENANCE-NOTE.md).
...
Requiere **Node.js 22+** (la v2.0.0 elevó el requisito mínimo con `node-web-audio-api` 2.0). No se necesitan controladores MIDI, puertos virtuales ni software externo.
...
| `registry_stats` | Estadísticas a nivel de la biblioteca: número total de canciones, géneros, dificultades |
...
| `practice_setup` | Velocidad, modo, ajustes de voz y comando CLI recomendados para una canción. |
...
| Herramienta | Qué hace |
|------|--------------|
| `play_song` | Reproducción a través de altavoces: canciones de la biblioteca o archivos .mid sin procesar. Cuatro motores (piano, voz, tracto vocal, guitarra), cualquier velocidad, modo, rango de compases, además de un metrónomo con conteo y una marca `record` que captura la sesión para la evaluación. Los motores sintetizador y en capas solo se pueden usar a través de la línea de comandos. |
...
| `view_piano_roll` | Renderizar como SVG (color de las notas o arcoíris cromático de clases de tono). |
...
| `preview_teaching_cues` | Ver todas las notas pedagógicas y los momentos clave antes de tocar. |

### Practicar

| Herramienta | Qué hace |
|------|--------------|
| `practice_loop` | El ejercicio que un profesor real asigna: repetir los compases 5–8 más lentamente, y el tempo aumenta (+5%) solo después de una ejecución *limpia*; cada ejecución se graba, evalúa y resume. |
| `practice_status` | Estado del ejercicio: ejecución actual, velocidad y diagnóstico por compás de la última interpretación. |
| `score_last_take` | Evaluar la interpretación más reciente grabada: precisión del tono, ritmo, integridad, veredicto por nota. |
| `view_scored_piano_roll` | La partitura marcada que utiliza todo profesor: el piano roll superpuesto con los veredictos por nota en una paleta segura para personas con daltonismo (sólido = correcto, punteado = ritmo, ✕ = error). |
...
| `ai_jam_sessions` | Generar un esquema para improvisar: progresión de acordes, esquema de la melodía y sugerencias de estilo para la reinterpretación. |

### Guitarra

| Herramienta | Qué hace |
|------|--------------|
| `view_guitar_tab` | Renderizar una tablatura interactiva de guitarra como HTML: hacer clic para editar, cursor de reproducción, atajos de teclado. |
...
| `score_annotation` | Calidad de las anotaciones en 5 dimensiones: integridad, profundidad, especificidad, valor pedagógico, vocabulario. |
...
| `list_sections` | Ver las secciones estructurales de una canción (Introducción, Verso, Coro, etc.). |
| `add_section` | Agregar un marcador de sección a una canción para la navegación estructural. |

### Indicaciones MCP

Tres plantillas de indicaciones para flujos de trabajo pedagógicos estructurados:
...
v2.0.0: la versión en la que el conjunto de datos demostró su utilidad (ver [CHANGELOG](CHANGELOG.md)). **Importante: la versión mínima de Node.js ahora es 22** (`node-web-audio-api` 2.0); la interfaz del programa en sí no ha cambiado: seis motores de sonido, 46 herramientas MCP, 3 plantillas de indicaciones y una **biblioteca totalmente anotada: 120/120 canciones de 12 géneros** (12 campos clave corregidos a las tonalidades detectadas en el contenido en esta versión). El ciclo pedagógico está cerrado de principio a fin: metrónomo con conteo → grabación en vivo → evaluación por nota → piano roll marcado → bucles de práctica que aumentan el tempo solo después de ejecuciones limpias. La interfaz del navegador es una herramienta de composición real: transporte preciso al ritmo con regiones de bucle, captura con activación de grabación, deshacer/rehacer completo, selección múltiple y portapapeles, soporte táctil — [disponible en la web](https://mcp-tool-shop-org.github.io/ai-jam-sessions/cockpit/).

También se publica **[jam-actions-v0](#training-dataset)**, un conjunto de datos de entrenamiento de 115 registros que contiene trazas del uso de herramientas MCP en múltiples interacciones sobre piezas clásicas para piano, con una puerta de liberación de 7 ejes, reproducibilidad en condiciones de inicio en frío y metadatos completos de Zenodo + CITATION.cff (CC-BY-SA-3.0-DE), que se encuentra replicado en [Hugging Face](https://huggingface.co/datasets/mcp-tool-shop/jam-actions-v0) y ahora incluye **resultados detallados del ajuste fino en ambas direcciones**: un resultado negativo honesto (v0) y un resultado positivo sujeto a una disciplina de preregistro que se detuvo justo antes de alcanzar su propio umbral de victoria (v1); consulte los [recibos del ajuste fino](#training-dataset). Esta versión también corrige los registros de Bach en la fuente (revisiones del conjunto de trabajo r001/r002 con correcciones) después de que la puerta de ejecución del flujo de trabajo v1 detectara que la ventana publicada excedía las 62 medidas reales de BWV 846. Se realizaron 2506 pruebas exitosas en el servidor MCP, la interfaz de usuario, los paquetes de conjuntos de datos, los sistemas de evaluación y el validador de la puerta de liberación. El MIDI está completo, cada canción puede servir como material didáctico y el corpus de aprendizaje se incluye con él.
...
**Datos NO accedidos (rutas predeterminadas):** el servidor y la interfaz de línea de comandos MCP no realizan llamadas a la red, no leen credenciales ni acceden a archivos del sistema fuera del directorio de canciones del usuario. No se recopila ni se envía ninguna telemetría. El **conjunto de herramientas/evaluación opcional** incluido en el mismo paquete (`scripts/run-llm-eval.ts`, verificador de procedencia) es la única excepción: cuando lo invoca explícitamente, puede llamar a las API de LLM (lee `ANTHROPIC_API_KEY` de su entorno, pero nunca lo almacena) y obtener URL de procedencia. Nunca se ejecuta como parte del servidor, la interfaz de línea de comandos o la instalación.

**Permisos:** el servidor MCP utiliza solo el transporte stdio (sin HTTP). La interfaz de línea de comandos accede al sistema de archivos local y a los dispositivos de audio. Consulte [SECURITY.md](SECURITY.md) para obtener la política completa.
