...
Un pianoforte e una chitarra che un'intelligenza artificiale impara a suonare. Non un sintetizzatore, né una libreria MIDI: è uno strumento didattico.
...
- **Lettura** — partiture MIDI reali con annotazioni musicali dettagliate. Non approssimazioni scritte a mano, ma partiture analizzate, interpretate e spiegate.
...
- **Canto** — sintesi del tratto vocale con 20 preset vocali, dall' soprano operistico a un coro elettronico. Modalità di accompagnamento con solfeggio, contorno e narrazione sillabica.
...
La rappresentazione grafica della tastiera (piano roll) è il modo in cui l'IA "vede" la musica. Converte qualsiasi brano in formato SVG: blu per la mano destra, corallo per la mano sinistra, con griglie di battute, dinamiche e confini delle misure:
...
- **7 sistemi di accordatura** — Temperamento equabile, intonazione giusta (maggiore/minore), pitagorico, meantone a quarto comma, Werckmeister III, o offset personalizzati in centesimi. Riferimento A4 regolabile (392–494 Hz).
- **Controllo dell'accordatura** — tabella delle frequenze, tester degli intervalli con analisi della frequenza delle battute e importazione/esportazione dell'accordatura.
- **Importazione/esportazione dello spartito** — serializza l'intero spartito in formato JSON e ricaricalo.
...
| Rock | Your Song (Elton John) | Si maggiore | Voic leading di pianoforte, inversioni, canto conversazionale. |
...
| **Sample Piano** | Riproduzione di file WAV | Pianoforte a coda Salamander — 480 campioni, 16 livelli di intensità, 88 tasti. La versione reale. |
...
| **Vocal Synth** | Sintesi additiva | 15 preset di voci "Kokoro" con modellazione della formante, respiro, vibrato. Deterministic (generatore di numeri casuali con seme). |
| **Guitar** | Sintesi additiva | Corda pizzicata modellata fisicamente — 4 preset (chitarra dreadnought in acciaio, classica in nylon, jazz archtop, a dodici corde), 8 accordature, 17 parametri regolabili. |
...
Sei voci di pianoforte regolabili, ognuna con parametri indipendenti (brillantezza, decadimento, durezza delle martellette, disaccordatura, ampiezza stereo e altro):
...
| Pianoforte brillante | Brillante, moderno, pop |

### Voci di chitarra

Quattro preset di voci di chitarra con sintesi di corde modellata fisicamente, ognuno con 17 parametri regolabili (brillantezza, risonanza della cassa, posizione della pizzicata, smorzamento delle corde e altro):
...
| Chitarra a dodici corde | Brillante, doppia, simile a un chorus |
...
Un file Markdown al giorno, salvato in `~/.ai-jam-sessions/journal/`. Leggibile dagli umani, solo in scrittura (append-only). Nella sessione successiva, l'intelligenza artificiale legge il suo diario e riprende da dove si era interrotta.
...
34 strumenti suddivisi in cinque categorie:

### Apprendimento

| Strumento | Cosa fa |
|------|--------------|
| `list_songs` | Ricerca per genere, difficoltà o parola chiave |
...
| `compare_songs` | Riconoscimento di schemi tra generi: relazioni tra tonalità, somiglianze di altezze/intervalli, forme comuni, collegamenti didattici. |
...
| Strumento | Cosa fa |
|------|--------------|
| `play_song` | Riproduzione tramite altoparlanti: brani della libreria o file .mid grezzi. Qualsiasi motore, velocità, modalità, intervallo di battute. |
| `stop_playback` | Ferma |
| `pause_playback` | Metti in pausa o riprendi |
| `set_speed` | Modifica la velocità durante la riproduzione (da 0.1x a 4.0x) |
| `playback_status` | Anteprima in tempo reale: battuta corrente, tempo, velocità, timbro della tastiera, stato. |
| `view_piano_roll` | Rendering in formato SVG (colorazione manuale o arcobaleno cromatico basato sull'altezza). |
| `score_performance` | Valutazione di una performance MIDI: accuratezza dell'intonazione, tempismo, completezza, con feedback graduato. |
...
| `ai_jam_sessions` | Genera una bozza di improvvisazione: progressione di accordi, schema melodico e suggerimenti di stile per la reinterpretazione. |
...
| `annotate_song` | Scrivi il linguaggio musicale per un brano grezzo e trasformalo in un brano pronto per l'uso. |
...
| `score_annotation` | Valutazione della qualità delle annotazioni in 5 dimensioni: completezza, profondità, specificità, valore didattico, vocabolario. |
...
Versione 1.1.0. Sei motori audio, 34 strumenti MCP, 120 brani in 12 generi con esempi accuratamente annotati. Editor interattivo di tablature per chitarra. Interfaccia utente per browser con 20 preset vocali, 10 timbri per strumenti, 7 sistemi di accordatura e un'API per l'analisi musicale. Visualizzazione della tastiera in due modalità di colore. Diario di pratica per l'apprendimento continuo. Valutazione delle performance MIDI, valutazione della qualità delle annotazioni e riconoscimento di schemi tra generi. Tutti i file MIDI sono inclusi: la libreria cresce man mano che l'intelligenza artificiale impara.

## Sicurezza e privacy

**Dati accessibili:** libreria di brani (JSON + MIDI), directory dei brani dell'utente (`~/.ai-jam-sessions/songs/`), configurazioni di accordatura per chitarra, voci del diario di pratica, dispositivo di output audio locale.
...
**Autorizzazioni:** Il server MCP utilizza solo il trasporto stdio (nessun HTTP). L'interfaccia a riga di comando accede al file system locale e ai dispositivi audio. Consultare il file [SECURITY.md](SECURITY.md) per la politica completa.
