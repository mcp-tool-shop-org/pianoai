...
**Mirror di HuggingFace.** In arrivo in una patch v1.4.x: vedere [`datasets/jam-actions-v0-public/publication-receipt.json`](datasets/jam-actions-v0-public/publication-receipt.json) per il blocco dello stato differito. Il DOI di Zenodo è l'identificatore di citazione canonico; lo specchio HF serve solo per la scoperta all'interno dell'ecosistema ML.
...
v1.5.0: la versione in cui ha imparato a insegnare (vedi [CHANGELOG](CHANGELOG.md)). Sei motori sonori, 46 strumenti MCP, 3 modelli di suggerimento e una **libreria completamente annotata: 120/120 canzoni in 12 generi**, ogni annotazione basata sull'analisi per canzone e con controllo della qualità. Il ciclo didattico è chiuso dall'inizio alla fine: metronomo con conteggio iniziale → registrazione dal vivo → valutazione per nota → partitura annotata → cicli di pratica che aumentano il tempo solo dopo esecuzioni perfette. La console del browser è diventata un vero strumento di composizione: trasporto preciso al ritmo con regioni di loop, acquisizione con attivazione della registrazione, annulla/ripeti completo, selezione multipla e area di ritaglio, supporto touch — ed è [disponibile sul web](https://mcp-tool-shop-org.github.io/ai-jam-sessions/cockpit/). Trasposizione delle canzoni, marcatori di sezione, disattivazione/isolamento della mano sinistra/destra, tablatura interattiva per chitarra, 7 sistemi di accordatura, diario di pratica, persistenza della sessione.

Pubblica anche **[jam-actions-v0](#training-dataset)**: un set di dati di addestramento di 115 record di tracce di utilizzo multi-turno degli strumenti MCP su pianoforte classico, con una soglia di rilascio a 7 assi, riproducibilità in condizioni iniziali fredde e metadati completi Zenodo + CITATION.cff (CC-BY-SA-3.0-DE) — ora disponibile anche su [Hugging Face](https://huggingface.co/datasets/mcp-tool-shop/jam-actions-v0). 2506 test superati sul server MCP + console + pacchetti di set di dati + strumenti di valutazione + validatore della soglia di rilascio. Il MIDI è tutto lì, ogni canzone può insegnare e il corpus di tale apprendimento viene fornito con essa.
