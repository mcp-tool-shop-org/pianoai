...
- **Ascolto:** sei motori audio (pianoforte oscillatore, pianoforte campionato, campioni vocali, tratto vocale fisico, sintetizzatore vocale additivo, chitarra modellata fisicamente) che suonano attraverso gli altoparlanti, in modo che le persone nella stanza diventino le orecchie dell'IA.
- **Visione:** una tastiera virtuale che visualizza ciò che è stato suonato come SVG, in modo che il modello possa leggerlo e verificarlo. Un editor interattivo di tablature per chitarra. Una console del browser con una tastiera visiva, un editor di note a doppia modalità e un laboratorio di accordatura.
- **Memoria:** un diario di pratica che persiste tra le sessioni, in modo che l'apprendimento si accumuli nel tempo.
- **Canto:** sintesi del tratto vocale con 20 preset vocali, dal soprano d'opera al coro elettronico. Modalità "sing along" con solfeggio, contorno e narrazione delle sillabe.
...
La stessa versione include il motore di composizione che alimenta il pannello (`src/compose/`: un gate deterministico per la conduzione delle voci con preset di stile denominati, specifiche di voicing basate sull'appartenenza per costruzione, un raffinatore parte per parte), un controllo completo dello stato di salute (45 problemi risolti: valuta di sicurezza, stringhe umanizzate, una modifica visiva che preserva l'aspetto), le voci della libreria Satie e Debussy rielaborate da byte pubblici con licenza Mutopia e un passaggio di rafforzamento del test con estranei: messaggi di errore descrittivi, buste di errore strutturate `{code, message, hint}`, un archivio tar curato, notifiche sullo stato di avanzamento degli strumenti lunghi e grammatica degli errori CLI. La superficie attiva è costituita da **49 strumenti e 4 modelli di prompt**, con **3.033 test superati (1 saltato)**. **Stato della pubblicazione:** l'ultima versione su npm è la **2.0.0**: tutto ciò che è più recente si trova su `@mcptoolshop/ai-jam-sessions@2.2.0` fino al rilascio della 2.2.0; nel frattempo, esegui da una copia clonata.
