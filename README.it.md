...
- **Ascolto:** sei motori audio (pianoforte a oscillatore, pianoforte a campioni, campioni vocali, tratto vocale fisico, sintetizzatore vocale additivo, chitarra modellata fisicamente) che riproducono il suono attraverso gli altoparlanti, trasformando così gli ascoltatori in una sorta di "orecchie" dell'IA.
- **Visione:** una tastiera virtuale che visualizza ciò che viene suonato in formato SVG, un formato che il modello può leggere e verificare. Un editor interattivo di tablature per chitarra. Un'interfaccia di controllo con una tastiera visiva, un editor di note a doppia modalità e un laboratorio di accordatura.
- **Memorizzazione:** un diario di pratica che conserva i dati tra le sessioni, in modo che l'apprendimento si accumuli nel tempo.
- **Canto:** sintesi del tratto vocale con 20 preset vocali, da soprano d'opera a coro elettronico. Modalità di accompagnamento con solfeggio, indicazione della melodia e pronuncia delle sillabe. E una vera linea melodica cantata, sincronizzata con il tempo del pianoforte: una partitura che guida il cantante, basata sul MIDI della canzone, con una regolazione del tempo (40 ms) e dell'intonazione (50 centesimi) prima che venga udita (vedi [Sing](#sing)).
...
**Una linea melodica cantata, sincronizzata con il tempo: il percorso vocale.** Qualsiasi brano della libreria può contenere una vera linea melodica cantata, sincronizzata con il pianoforte: un **clock della partitura** (`scripts/build-score-clock.mjs`) ricava l'intonazione, l'inizio e la durata di ogni sillaba dal MIDI della canzone, sulla base della linea temporale del brano; un cantante locale, con licenza Apache-2.0 e guidato dalla partitura ([SoulX-Singer](https://github.com/Soul-AILab/SoulX-Singer)), canta seguendo questo clock sulla tua GPU; e due gate misurano l'output prima che venga considerato un mix: **tempo**: ogni inizio di vocale entro 40 ms rispetto alla partitura; **intonazione**: ogni nota entro 50 centesimi, con un offset globale di 20. Le parole vengono scelte da un insieme di registrazioni e unite solo ai confini delle parole, con dissolvenze incrociate. Parametri di controllo: `--track` (quale traccia MIDI è la melodia; `--list-tracks` per visualizzare), `--lyrics "A-ma-zing grace …"` (un token per nota, le sillabe unite da `-`), `--measures`, il clip di prompt (la voce), il numero di registrazioni e le soglie del gate, ciascuno con la sua citazione in `scripts/vocal_clock.py`. Percorso, parametri e risultati: [manuale → Voci](https://mcp-tool-shop-org.github.io/ai-jam-sessions/handbook/vocals/), [`docs/vocal-clock.md`](docs/vocal-clock.md); la ricerca alla base delle scelte: [`docs/vocal-singing-study-2026-09.md`](docs/vocal-singing-study-2026-09.md).
...
**v2.3.0: la versione in cui lo strumento ha imparato a cantare, sincronizzato con il tempo** (vedi [CHANGELOG](CHANGELOG.md)). Ora, qualsiasi brano della libreria può contenere una vera linea melodica cantata, sincronizzata con il pianoforte: un **clock della partitura** ricava l'intonazione, l'inizio e la durata di ogni sillaba dal MIDI della canzone, sulla base della linea temporale del brano; un cantante locale, con licenza Apache-2.0 e guidato dalla partitura ([SoulX-Singer](https://github.com/Soul-AILab/SoulX-Singer)), canta seguendo questo clock; e due gate misurano l'output prima che venga considerato un mix: tempo (ogni vocale entro 40 ms rispetto alla partitura) e intonazione (ogni nota entro 50 centesimi). L'esecuzione di Amazing Grace inclusa misura un massimo di 6 ms di errore nel tempo e -2,7 centesimi di errore nell'intonazione, con i dati registrati; la pagina di destinazione la presenta come uno stato reale, con l'unico difetto rimanente indicato (la giunzione iniziale). Il percorso, i suoi parametri e la ricerca alla base di ogni scelta (cinque aree di studio, citate) sono disponibili nel [manuale](https://mcp-tool-shop-org.github.io/ai-jam-sessions/handbook/vocals/) e [`docs/`](docs/). L'interfaccia utente è rimasta invariata, con **49 strumenti e 4 modelli di prompt**, con **3.080 test superati (1 saltato)**, oltre alla suite pytest dello strumento vocale. **Stato della pubblicazione:** pubblicato — [`@mcptoolshop/ai-jam-sessions@2.3.0`](https://www.npmjs.com/package/@mcptoolshop/ai-jam-sessions) su npm, con tracciabilità verificata.
...
La stessa versione include il motore di composizione che alimenta il pannello (`src/compose/`: un gate deterministico per la conduzione delle voci con preset di stile denominati, specifiche di voicing basate sull'appartenenza, un raffinatore che lavora su una parte alla volta), un controllo completo dello stato di salute (45 problemi risolti: sicurezza, stringhe umanizzate, una modifica visiva che preserva l'aspetto), le voci della libreria di Satie e Debussy, riprese da dati di Mutopia di pubblico dominio con tracciabilità verificata, e un passaggio di rafforzamento dei test con utenti esterni: messaggi di errore descrittivi, buste di errore strutturate `{code, message, hint}`, un archivio tar curato, notifiche di avanzamento per gli strumenti che richiedono molto tempo e grammatica degli errori per la CLI. Questa versione è stata pubblicata come [`@mcptoolshop/ai-jam-sessions@2.2.0`](https://www.npmjs.com/package/@mcptoolshop/ai-jam-sessions) con 49 strumenti, 4 modelli di prompt e 3.033 test.
