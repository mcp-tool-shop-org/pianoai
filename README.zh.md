...
- **聆听**——六个音频引擎（振荡器钢琴、采样钢琴、人声样本、物理人声声道、加法人声合成器、物理建模吉他），通过您的扬声器播放，使房间里的人成为人工智能的“耳朵”。
- **观看**——一个钢琴卷帘，它将播放的内容渲染为 SVG，模型可以读取并验证。一个交互式吉他谱编辑器。一个浏览器控制面板，带有视觉键盘、双模式音符编辑器和调音实验室。
- **记忆**——一个练习日志，它会跨会话保存，因此学习效果会随着时间的推移而累积。
- **歌唱**——带有 20 种人声预设的人声声道合成，从歌剧女高音到电子合唱。带有音阶、轮廓和音节叙述的伴唱模式。以及在钢琴的时钟上进行真实的演唱：一个基于歌曲 MIDI 驱动的、受乐谱影响的歌手，在您听到之前，会根据时间（40 毫秒）和音高（50 厘）进行门限控制——请参见[Sing](#sing)。
...
长期以来，该服务器可以发出声音，但无法对其进行分析。模型播放音乐，人类聆听，然后模型根据人类的反馈进行调整。现在，这个差距已经弥合。

只需将其指向一个 WAV 文件，它就能测量其中的内容。它不是通过查看图像并进行猜测，而是通过将信号传递到与它已经用于乐谱分析的工具相同的工具中：

- **`analyze_audio`** — 提取起始点、音高轮廓和音量。音高会以音符名称和音高偏差的形式返回，而不是原始频率。
- **`transcribe_audio`** — 将录音转换为音符：音高、起始时间、持续时间和每个音符与标准音高的距离。
- **`score_audio_take`** — 通过“听”的方式，将演奏与库中的歌曲进行比较。它会转录录音，将其与乐谱进行匹配，并报告哪些音符演奏正确，哪些音符偏离，以及哪些音符遗漏。然后，`view_scored_piano_roll`会将结果绘制在乐谱上，就像它对捕获的 MIDI 音轨所做的那样。这就是评估真实乐器、人声或任何没有 MIDI 记录的情况的方法。
- **`view_spectrogram`** — 查看声音。一个恒定 Q 值的频谱图，左侧边缘有一排钢琴键盘，因此可以一目了然地读取音高，并且可以根据要求在频谱图上绘制歌曲的预期音符。

**它不会告诉你什么。** 该图像用于查找*哪里*出了问题；每个数字都来自信号处理，而不是来自模型读取图像。转录器一次处理一行，因此和弦或完整的混音会产生一个确定的错误结果，并且会明确指出这一点。在最先进的技术中，起始点检测的准确率约为 F1 0.88，因此“遗漏”的音符可能是转录器无法听到的音符，而不是你没有演奏的音符——这些工具会在自己的输出中包含此警告，而不是将其隐藏在这里。

整个系统都是独立的：转换、音高跟踪器、起始点检测器、WAV 解码器和 PNG 编码器都包含在此存储库中，并且它们在 Node 和浏览器中产生相同的数字。
