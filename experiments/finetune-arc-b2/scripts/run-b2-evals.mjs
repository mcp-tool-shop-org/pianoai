#!/usr/bin/env node
// ─── run-b2-evals.mjs — Finetune Arc B-2 sequential sealed-eval runner ────────
//
// P0-LOCK-b2 §6/§9/§15. Runs the pre-run gate, then exactly six sealed
// evaluations (fresh baseline first, then the five B-2 seeds ascending), one at
// a time, abort-on-first-failure, on the B-2 abstain surface (--abstain-surface).
// One eval per model: a completed results file is never regenerated (crash-resume
// within a run rides the harness's own checkpoint file). All flags byte-pinned.
//
// Adapted from finetune-arc-v2/scripts/run-b1-evals.mjs — deltas: b2 tags, single
// finetune-arc-b2/artifacts dir (one pod, no podA/podB), --abstain-surface, b2
// evals dir. Same 36-record cohort (--sample-filter b1-confirm-cohort), e3+e3-tool
// n=3, ollama backend.
//
// Usage:  node experiments/finetune-arc-b2/scripts/run-b2-evals.mjs
// Exit 0 when all six artifacts exist and are complete; 1 on any gate or run
// failure (ANDON — fix or amend the lock, never patch silently).
// ─────────────────────────────────────────────────────────────────────────────
...
const B2_DIR = join(REPO_ROOT, "experiments", "finetune-arc-b2");
const EVALS_DIR = join(B2_DIR, "evals");
const LOG_PATH = join(EVALS_DIR, "b2-run.log");
const GATE_RECEIPT = join(EVALS_DIR, "b2-prerun-gate.json");
const P4_RECEIPT = join(B2_DIR, "artifacts", "p4-receipt.json");
const ARTIFACT_ROOT = join(B2_DIR, "artifacts");

const MODELS = [
  { label: "baseline", tag: "qwen2.5:7b" },
  { label: "seed13", tag: "jam-ft-b2-qwen25:seed13" },
  { label: "seed42", tag: "jam-ft-b2-qwen25:seed42" },
  { label: "seed271", tag: "jam-ft-b2-qwen25:seed271" },
  { label: "seed512", tag: "jam-ft-b2-qwen25:seed512" },
  { label: "seed1024", tag: "jam-ft-b2-qwen25:seed1024" },
];
...
// ─── Pre-run gate (P0-LOCK-b2 §15; mirrors the B-1 gate) ──────────────────────

log("=== B-2 pre-run gate ===");

// (v) Lock committed and unmodified BEFORE any model call.
const lockLog = run("git", ["log", "--oneline", "-1", "--", "experiments/finetune-arc-b2/P0-LOCK.md"]);
if (!lockLog.stdout.trim()) fail("P0-LOCK.md is not committed — commit the lock before any model call");
const lockDirty = run("git", ["status", "--porcelain", "--", "experiments/finetune-arc-b2/P0-LOCK.md"]);
...
// (ii) Cohort derivation ↔ harness const equality (B-2 reuses the b1-confirm-cohort).
const cv = run("pnpm", [
  "exec", "tsx", "experiments/finetune-arc-v2/scripts/derive-b1-cohort.ts", "--verify",
]);
if (cv.status !== 0) fail(`cohort verify failed:\n${cv.stdout}\n${cv.stderr}`);
log("cohort derivation ↔ harness const (b1-confirm-cohort): PASS");

// (iii) B-2 GGUF artifacts match p4-receipt shas.
const p4 = JSON.parse(readFileSync(P4_RECEIPT, "utf8"));
if (p4.models.length !== 5) fail(`p4-receipt has ${p4.models.length} models, expected 5 seeds`);
...
// (Windows gotcha carried from B-1: a down daemon auto-starts and inherits our
// stdio pipes, so spawnSync may timeout-kill with the full modelfile already
// buffered — validate on captured output, FROM present == tag resolvable.)
const tagEvidence = [];
...
writeFileSync(
  ...
  JSON.stringify(
    {
      schema: "finetune-arc-b2-prerun-gate/1.0.0",
      generated_at: new Date().toISOString(),
      lock_commit: lockLog.stdout.trim(),
      package_checksums: "PASS",
      cohort_verify: "PASS",
      eval_surface: "abstain (--abstain-surface)",
...
// ─── Sequential sealed runs (B-2 abstain surface) ─────────────────────────────
...
for (const m of MODELS) {
  const out = join(EVALS_DIR, `b2-${m.label}-results.json`);
  const sample = join(EVALS_DIR, `b2-${m.label}-sample.json`);
  if (isComplete(out)) {
    log(`SKIP ${m.label} (${m.tag}) — completed results artifact already exists (one eval per model)`);
    continue;
  }
  log(`=== sealed eval (abstain surface): ${m.tag} -> b2-${m.label}-results.json ===`);
  const t0 = Date.now();
  const r = spawnSync(
    "pnpm",
    [
