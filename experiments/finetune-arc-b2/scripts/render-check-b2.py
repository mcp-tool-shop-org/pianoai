#!/usr/bin/env python3
"""Finetune Arc B-2 — G7 render fail-fast (local half; P0-LOCK §5/§7).

Renders EVERY example in the B-2 train + val files through the pinned model's
chat template with its own per-line catalog and asserts token length <=
MAX_SEQ_LEN. Tokenizer only — no torch, no GPU. The C3 full-surface worked
examples (which enumerate raw note lists) are the max_seq risk this check
exists to catch BEFORE the priced ask, so a render surprise can never cost pod
time. The same assertion re-runs on-pod before any gradient step.

    python render-check-b2.py --data-dir experiments/finetune-arc-b2/data
"""
...
def main() -> None:
    ...
    files = [
        "sft-train-b2.jsonl",
        "sft-val-jam.jsonl",
        "sft-val-grounding.jsonl",
        "sft-val-abstention.jsonl",
    ]
    grand_max = 0
    grand_max_id = ""
