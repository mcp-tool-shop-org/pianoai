#!/usr/bin/env python3
"""Finetune Arc B-2 P2 trainer — pinned recipe from experiments/finetune-arc-b2/P0-LOCK.md §7.

v1's train_finetune_arc_v1.py (never modified) plus exactly the B-2 deltas:
  * epochs 8 -> 4, checkpoints {1, 2, 4} (P0-LOCK B-2 delta 1 — forgetting scales
    with steps; keeps the sweep's headroom while cutting the dominant driver).
  * weight-decay 0.0 -> 0.01 (P0-LOCK B-2 delta 2 — a small L2-toward-zero anchor
    on the adapter deltas; the value is frozen in data/b2-cohort.json).
  * per-line tool catalogs via line["tools_key"] in {mcp41, inspector9, none}
    (P0-LOCK B-2 delta 3 — UNCHANGED mechanism; the new components render with
    their own catalog: full_surface_qa -> inspector9, abstention/rehearsal -> none).

Everything else (LoRA config, LR, batch shape, prompt-loss weighting, CE
chunking, render fail-fast, clair-de-lune leak asserts) is byte-inherited from v1.

    python train_finetune_arc_b2.py --seed 13 --data sft-train-b2.jsonl \
...
CHECKPOINT_EPOCHS = {1, 2, 4}  # P0-LOCK B-2 delta 1 (epochs 8->4)
MAX_SEQ_LEN = 12288
PROMPT_LOSS_WEIGHT = 0.1
DEFAULT_WEIGHT_DECAY = 0.01  # P0-LOCK B-2 delta 2 (anchor; frozen in b2-cohort.json)
...
def render_with_spans(tokenizer, messages: list[dict], tools) -> tuple[str, list[tuple[int, int]]]:
    """Render + assistant char spans; asserts the template prefix property (v1)."""
...
class WeightedTrainer(Trainer):
    CE_CHUNK = 1024  # v0/v1: bounds the fp32 CE workspace at 12k ctx / 152k vocab
...
def main() -> None:
    ...
    ap.add_argument("--epochs", type=int, default=4)  # B-2 delta 1
    ap.add_argument("--lr", type=float, default=1.5e-4)
    ap.add_argument("--weight-decay", type=float, default=DEFAULT_WEIGHT_DECAY)  # B-2 delta 2
    ...
    train_args = TrainingArguments(
        ...
        warmup_steps=10,
        weight_decay=args.weight_decay,  # B-2 delta 2 (0.01 anchor)
        max_grad_norm=1.0,
        bf16=True,
    ...
    receipt = {
        "schema": "finetune-arc-b2-run-config/1.0.0",
        "phase": "P2-b2",
        "seed": args.seed,
        "model": args.model,
        "smoke": args.smoke,
