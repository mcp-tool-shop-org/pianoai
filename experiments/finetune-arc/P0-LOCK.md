...
| Seeds | **{13, 42, 271, 512, 1024}** (A3 cancelled A2; amendment A6 restored 5 seeds ex-ante — director-approved, before any sealed eval) | L8 |
...
**Amendment A6 (2026-07-10, director-approved with an explicit priced ask — "yes"):** seeds 512 + 1024 restored, bringing the matrix to the dispatch-preferred **5 seeds**. Decision taken **before any sealed-eval result existed** (ex-ante; not adaptive sampling). Execution: second parallel A100 SXM pod `3f331akiiqw4xu` running the identical pod_run_v2.sh with `SEEDS=(512 1024)` (one-line sed, verified before launch); its own per-seed streaming fetcher (→ `artifacts/podB/`), its own 7 h dead-man (~$10.43 absolute cap; estimate ≈ $8.70). Each pod's P3 selects per-seed on the same inner split with the same deterministic rule — selection is seed-independent, so split-pod selection is equivalent to unified selection. P5 runs once per seed across all five; P6 pools all five, no best-of-seeds.
