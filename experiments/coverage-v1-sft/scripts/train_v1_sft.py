...
# v1 splits by song across 27 songs, not one held-out phrase, so there is no
# single id to guard. The corpus's own assertNoStraddle test owns that
# property; this trainer asserts only that the split column is honoured.
HELDOUT_SONG_ID = None
...
def load_lines(data_path: Path) -> list[dict]:
    ...
    # Whatever the split rule, a file handed to the trainer must be one side of
    # it. A mixed file is the cheapest way to train on the test set by accident.
    splits = {l.get("split") for l in lines}
    if splits != {"train"}:
        raise AssertionError(f"training data is not all train split: found {sorted(splits)}")
    return lines
...
def main() -> None:
    ...
    receipt = {
        "schema": "coverage-v1-run-config/1.0.0",
        "seed": seed,
        "model": model_name,
        "smoke": args.smoke,
        "tool_catalog": {
