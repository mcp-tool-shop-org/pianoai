#!/usr/bin/env bash
# Finetune Arc v1 P4b (local) — register per-seed Q4_K_M GGUFs as ollama models
# with TEMPLATE + SYSTEM copied byte-identical from the baseline qwen2.5:7b tag
# (P0-LOCK-v1 §9; v0's p4-make-ollama-models.sh adapted: v1 tag names, scans
# artifacts/podA + artifacts/podB, rerunnable as pods land — the receipt is
# rewritten with every tag present at run time).
#
#   bash experiments/finetune-arc-v1/scripts/p4-make-ollama-models-v1.sh
set -euo pipefail
...
RECEIPT='{"schema":"finetune-arc-v1-p4-receipt/1.0.0","baseline_tag":"qwen2.5:7b","baseline_quant":"'$BASELINE_QUANT'","models":['
FIRST=1
for GGUF in artifacts/podA/jam-ft-v1-qwen25-seed*.q4_k_m.gguf artifacts/podB/jam-ft-v1-qwen25-seed*.q4_k_m.gguf; do
  [ -f "$GGUF" ] || continue
  BASE=$(basename "$GGUF")
  SEED=$(echo "$BASE" | sed -E 's/jam-ft-v1-qwen25-(seed[0-9]+)-.*/\1/')
  EPOCH=$(echo "$BASE" | sed -E 's/.*-(epoch[0-9]+)\.q4_k_m\.gguf/\1/')
  NAME="jam-ft-v1-qwen25:$SEED"
...
echo "P4-v1 receipt -> $OUT"
ollama list | grep jam-ft-v1 || true
