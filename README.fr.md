...
Un LLM (Large Language Model) peut lire et écrire du texte, mais il ne peut pas expérimenter la musique de la même manière que nous. Il n'a pas d'oreilles, de doigts, ni de mémoire musculaire. AI Jam Sessions comble ce fossé en offrant au modèle des sens qu'il peut réellement utiliser :

- **Lecture** : de véritables partitions MIDI avec des annotations musicales détaillées. Pas de simples approximations manuscrites, mais des éléments analysés, expliqués et interprétés.
- **Audition** : six moteurs audio (piano à ondes, piano échantillonné, échantillons vocaux, tractus vocal physique, synthétiseur vocal additif, guitare modélisée physiquement) qui produisent le son via vos haut-parleurs, permettant aux humains présents de devenir les "oreilles" de l'IA.
...
- **Chant** : synthèse du tractus vocal avec 20 préréglages de voix, de la soprano d'opéra à la chorale électronique. Mode de chant avec narration de la gamme, du contour et des syllabes.
...
Le piano roll est la façon dont l'IA perçoit la musique. Il affiche n'importe quelle chanson au format SVG : bleu pour la main droite, corail pour la main gauche, avec des grilles de rythme, des dynamiques et des limites de mesures :
...
- **Pavé de piano bi-mode** — Basculez entre le mode Instrument (couleurs chromatiques) et le mode Voix (les notes sont colorées en fonction de la forme de la voyelle : /a/ /e/ /i/ /o/ /u/).
- **Clavier visuel** — Deux octaves de Do4, mappés à votre clavier QWERTY. Cliquez ou tapez.
- **20 préréglages de voix** — 15 voix mappées à Kokoro (Aoede, Heart, Jessica, Sky, Eric, Fenrir, Liam, Onyx, Alice, Emma, Isabella, George, Lewis, plus chœur et voix synthétiques), 4 voix mappées à des pistes, et une section de chœur synthétique.
- **10 préréglages d'instruments** — Les 6 voix de piano côté serveur, plus synthé, orgue, cloche et cordes.
...
- **Audit d'accordage** — Tableau de fréquences, test d'intervalles avec analyse de la fréquence des battements, et exportation/importation de l'accordage.
- **Importation/exportation de partitions** — Sériailisez la partition entière au format JSON et chargez-la.
...
| Classique | Für Elise (Beethoven) | La mineur | Forme de rondo, différenciation du toucher, discipline du pédalier. |
...
| Jazz | Autumn Leaves (Kosma) | Sol mineur | Progressions ii-V-I, notes guides, huitièmes de note swing, voicings sans fondamentale. |
...
| Ragtime | The Entertainer (Joplin) | Do majeur | Basse "oom-pah", syncopation, forme à plusieurs couplets, discipline du tempo. |
...
| Soul | Lean on Me (Bill Withers) | Do majeur | Mélodie diatonique, accompagnement gospel, réponse et question. |

Les chansons progressent de **brutes** (MIDI uniquement) → **annotées** → **prêtes** (entièrement jouables avec un langage musical). L'IA fait progresser les chansons en les étudiant et en rédigeant des annotations avec `annotate_song`.
...
| **Vocal Tract** | Modèle physique | Trombone Pink — Forme d'onde glottale basse fréquence passant à travers un guide d'ondes numérique à 44 cellules. Quatre presets : soprano, alto, ténor, basse. |
| **Vocal Synth** | Synthèse additive | 15 presets de voix Kokoro avec modelage de la formante, souffle, vibrato. Déterministe (générateur de nombres aléatoires initialisé). |
| **Guitar** | Synthèse additive | Cordes pincées modélisées physiquement — 4 presets (acier dreadnought, nylon classique, jazz archtop, douze cordes), 8 accordages, 17 paramètres réglables. |
...
| Piano électrique | Soie, jazz, ambiance Fender Rhodes |
...
Quatre presets de voix de guitare avec synthèse de cordes modélisées physiquement, chacun avec 17 paramètres réglables (brillance, résonance du corps, position de pincement, amortissement des cordes, etc.) :
...
| Twelve-String | Chatoyant, doublé, type chorus |
...
Un fichier Markdown par jour, stocké dans `~/.ai-jam-sessions/journal/`. Lisible par l'humain, ajout uniquement. Lors de la prochaine session, l'IA lit son journal et reprend là où elle s'était arrêtée.
...
| `registry_stats` | Statistiques de toute la bibliothèque : nombre total de chansons, genres, difficultés |
...
| Outil | Ce qu'il fait |
|------|--------------|
| `play_song` | Lecture via les haut-parleurs : morceaux de la bibliothèque ou fichiers .mid bruts. Possibilité de choisir le moteur, la vitesse, le mode et la plage de mesures. |
| `stop_playback` | Arrêt |
...
| `list_guitar_voices` | Présets de timbres de guitare disponibles. |
| `list_guitar_tunings` | Systèmes d'accordage de guitare disponibles (standard, drop-D, open G, DADGAD, etc.). |
| `tune_guitar` | Ajustement de n'importe quel paramètre de n'importe quel timbre de guitare. Les paramètres sont conservés entre les sessions. |
...
| Outil | Ce qu'il fait |
|------|--------------|
| `add_song` | Ajout d'un nouveau morceau au format JSON. |
| `import_midi` | Importation d'un fichier .mid avec des métadonnées. |
| `annotate_song` | Écriture d'un langage musical pour un morceau brut et conversion en format prêt à l'emploi. |
...
| `tune_keyboard` | Ajustement de n'importe quel paramètre de n'importe quel timbre de clavier. Les paramètres sont conservés entre les sessions. |
...
**Données concernées :** bibliothèque de morceaux (JSON + MIDI), répertoire des morceaux de l'utilisateur (`~/.ai-jam-sessions/songs/`), configurations d'accordage de guitare, entrées du journal de pratique, périphérique de sortie audio local.

**Données NON concernées :** aucune API cloud, aucune information d'identification de l'utilisateur, aucune donnée de navigation, aucun fichier système en dehors du répertoire des morceaux de l'utilisateur. Aucune télémétrie n'est collectée ni envoyée.
