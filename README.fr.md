...
**Est-ce que cela permet réellement d’entraîner quelque chose ? — les résultats du réglage fin.** Les affirmations concernant l’ensemble de données sont testées de manière rigoureuse : les réglages fins préenregistrés sont comparés à une base de référence scellée, et les règles d’intégrité sont figées avant tout entraînement. **v0** (les 78 pistes seules) a donné un *résultat négatif honnête* — la QA basée sur des outils a chuté de 0,661 à 0,601 ([rapport](docs/finetune-arc-eval-report.md)). **v1** (un ensemble de données de 494 exemples ajoutant des pistes vérifiées par exécution et adaptées au contexte) a amélioré la même métrique de 0,661 à **0,863** (+0,202, permutation p = 0,0043, les cinq valeurs initiales supérieures à la base de référence, la chanson inédite +0,433) — et est toujours proposé comme étant *"globalement meilleur, mais sous-optimal"* parce que 12 des 16 comparaisons par paire n’ont pas atteint le seuil de victoire préenregistré de ≥13/16 d’une seule unité ([rapport](docs/finetune-arc-v1-eval-report.md)). Aucun adaptateur n’est publié à partir d’un résultat presque réussi. Les deux séries, les paramètres, les modifications et les résultats par valeur initiale sont disponibles dans [`experiments/`](experiments/) — la rigueur est l’objectif.
...
Nécessite **Node.js 22+** (la version v2.0.0 a augmenté le niveau minimal avec `node-web-audio-api` 2.0). Pas de pilotes MIDI, pas de ports virtuels, pas de logiciels externes.
...
v2.0.0 — la version dans laquelle l’ensemble de données a prouvé sa rigueur (voir [CHANGELOG](CHANGELOG.md)). **Modification importante : le niveau minimal de Node.js est maintenant 22** (`node-web-audio-api` 2.0) ; l’interface elle-même n’a pas changé — six moteurs sonores, 46 outils MCP, 3 modèles d’invite et une **bibliothèque entièrement annotée : 120/120 chansons dans 12 genres** (12 champs clés corrigés pour correspondre aux tonalités détectées dans le contenu de cette version). La boucle d’apprentissage est fermée de bout en bout : métronome avec compte à rebours → enregistrement en direct → notation par note → partition de piano annotée → boucles d’entraînement qui augmentent progressivement le tempo uniquement après des passages réussis. Le cockpit du navigateur est un véritable outil de composition — transport précis au rythme, capture avec activation de l’enregistrement, annulation/rétablissement complets, sélection multiple et presse-papiers, prise en charge tactile — [disponible en ligne](https://mcp-tool-shop-org.github.io/ai-jam-sessions/cockpit/).

Publie également **[jam-actions-v0](#training-dataset)** — un ensemble de données d’entraînement de 115 enregistrements sur l’utilisation d’outils MCP en plusieurs étapes pour le piano classique, avec une grille de publication à 7 axes, une reproductibilité en démarrage à froid et des métadonnées Zenodo + CITATION.cff complètes (CC-BY-SA-3.0-DE) — mises en miroir sur [Hugging Face](https://huggingface.co/datasets/mcp-tool-shop/jam-actions-v0), et contenant désormais **les résultats du réglage fin dans les deux sens** : un résultat négatif honnête (v0) et un résultat positif respectant la rigueur de l’enregistrement préalable, qui n’a pas atteint le seuil de victoire propre à cette version d’une seule comparaison par paire (v1) — voir les [résultats du réglage fin](#training-dataset). Cette version corrige également les enregistrements de Bach à la source (révisions de l’ensemble de travail r001/r002 avec errata) après que la grille de publication du pipeline v1 a détecté que la fenêtre publiée dépassait le nombre réel de mesures (62) de BWV 846. 2506 tests réussis sur le serveur MCP + cockpit + ensembles de données + outils d’évaluation + validateur de la grille de publication. Le MIDI est entièrement présent, chaque chanson peut servir à l’apprentissage et le corpus de cet apprentissage est fourni avec.
