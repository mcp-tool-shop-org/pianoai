...
**The P7-class director gate is now OPEN and awaiting the explicit yes** (the gate that never fired in v1): publish a v1 adapter (HF, beside the dataset) + the coordinator-authored results section on the landing page/handbook. Per the lock, this report's existence does not authorize either action — the director decides with the result in view. If it fires, seed selection for the published artifact and its compensator table are prepared under the gate, not before it.
