...
### Follow-up applied (2026-07-10, director-approved)

The receipted correction pass ran, against **freshly regenerated `--analyze` evidence rather than this addendum's table** — which mattered: `hoochie-coochie-man`'s source MIDI was replaced after this addendum was written (docs/library-data-audit.md, fragment → full 51-measure band transcription), and on the new source the stated A major fits at 0.446 with detection at noise margin (0.051), so **its recommendation above is void and the field was left alone**. The other four receipted corrections held on fresh evidence and were applied: shenandoah F→Eb (post-fix fit 0.88), the-water-is-wide C→G (0.761), amazing-grace G→Eb (0.362; detected label is C minor, the relative — window quality adjudicates Eb per the table above), everyday-i-have-the-blues Bb→Em (0.73).

The director also approved extending the same discipline to the D2 bulk waves' documented mismatches. Eight more fields were corrected, each on strong-margin, one-directional window evidence: peacherine-rag C→Eb (post-fix fit 0.922), dont-stop-believin E→Db (0.836), nuvole-bianche and nuvole-bianche-na Em→Ab (0.883 each), watermark Ab→E (0.84), kiss-the-rain Db→G (0.777, the everyday-i-have-the-blues precedent — one-directional windows over a diluted whole-song margin), corcovado Am→Bm (0.804), killing-me-softly Eb→Db (0.913). Left flagged, not corrected: forrest-gump (stated C fits 0.522 — tonic/dominant pair, the all-of-me precedent), metamorphosis-two (its 5 windows are all E major, which reads as stated A minor's dominant at least as well as detected B major's IV), opening-glassworks (thin, per D2-B's own "named as thin, not confirmed"), plus every relative-pair, modal, and zero-window case above.

In all 12 corrected configs the annotation prose was reframed to historical-label wording ("the X label this transcription originally carried…") so every mismatch narrative and every quoted number stays true now that the field matches content. Verification: all 12 re-analyzed as confirmed (stated = detected or exact enharmonic, fits 0.36–0.92), all 12 re-scored ≥80 (86–98), all 8 affected genres `--report` 10/10 ready, superlative-ban grep zero hits, full suite (2506 tests) + smoke 48/48 green.
