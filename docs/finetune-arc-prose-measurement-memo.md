...
|---|---|---|---|
| **Calibrated abstention / refusal** | **NO.** Forced single-letter A–E MCQ + an "E) cannot be determined" option is a **known-degenerate** instrument for abstention on three independent grounds (below). Reproduced at $0: abstention = **0.000 in every cell of all 6 models** → zero discriminative power. | **YES.** Abstention is SFT-trainable on a 7B, and B-2's *own* free-form inner-val proxy (0.75–0.88) shows the behavior was in fact installed — it was invisible only on the MCQ. | **RE-INSTRUMENT, then re-decide at $0.** Do **not** abandon the *behavior*; **do** abandon the MCQ-"E" *instrument*. Move to free-form decline detection and/or verbalized-confidence / P(True). Gate any future arc on a $0 baseline-non-degeneracy probe (§6.1). |
...
| EXTERNAL_VERIFIER | 2 | Existence oracle = deterministic arXiv API retrieval, mechanism-diverse from the four generating agents (which are *inputs*, not verifiers); 35/35 arXiv + 1 ACL resolved. **Honest ceiling:** the family-different *groundedness* lens (`prism verify`) was **not** run this session — deferred/advisory, the same disposition the v1 and B-2 locks took. Load-bearing claims are phrased to be robust to minor figure imprecision. |
...
Four parallel lanes, study-swarm protocol (named papers, years, arXiv IDs, one-sentence findings; every citation retrieved, not recalled). Existence oracle: **35/35 arXiv + 1 ACL resolved, 0 fabricated** (receipt beside this memo). Groundedness lens advisory/deferred (§Standards).
...
5. **P(True)/P(IK) — a model scoring the probability its own answer is correct / that it knows — is a continuous, reasonably-calibrated, thresholdable quantity that improves with scale.** Kadavath et al. 2022 (arXiv:2207.05221). A logit-self-eval instrument yields a graded, non-degenerate baseline out of the box.
...
Note: the B-2 sealed artifacts store only `selectedOptionIndex`/token counts, **not raw text**, so this gate cannot be re-derived from them — it needs a fresh *base-model* free-form pass (still $0, local). This memo specifies the gate; executing it is the director's call (it is the first step of validating, not running, a future arc).
...
1. **Abstention: do not abandon the behavior — abandon the instrument.** It is trainable (11–14) and B-2 almost certainly already installed it (free-form proxy 0.75–0.88); it was invisible only because MCQ-"E" cannot see it (1,2,4,15–19), a fact reproduced at $0 here (flat 0.000 across ~9,600 runs). **Before any future abstention arc, run the $0 base-model free-form non-degeneracy gate (§6.1).** If the base model already declines the unanswerable more than the answerable, the target is measurable and worth an arc *on that instrument*; if not, drop it. Either way, the decision is made at $0.
...
- **Citation existence oracle:** [finetune-arc-prose-measurement.citation-receipt.json](finetune-arc-prose-measurement.citation-receipt.json) — arXiv API `id_list`, **35/35 arXiv resolved, 0 fabricated**, + Rodriguez et al. 2021 verified via ACL Anthology = **36/36 sources**. Groundedness lens advisory/deferred (§Standards, EXTERNAL_VERIFIER).
